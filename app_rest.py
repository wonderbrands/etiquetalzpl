...
class RestAPI:
	...
	def update_imprimio_etiqueta_meli_picking(self, so_name):
			try:
				data = {
				'model': "stock.picking",
				'domain': json.dumps([['origin', '=',so_name], ['name', 'like', 'WH/PICK']]),
				'fields': json.dumps(['id','name','origin','imprimio_etiqueta_meli']),
				#'fields': json.dumps([]),
				}
				...
				print ('Error No marco la impresión en ODOO para le Picking: '+str(e) + '| '+str(so_name))
				return False
...
def imprime_zpl(so_name, ubicacion):
	...
	try:
		...
		resultado_pick = api.update_imprimio_etiqueta_meli_picking(so_name)
